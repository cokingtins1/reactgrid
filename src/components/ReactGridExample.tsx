...
const styledRanges = [
	...
	{
		range: {
			start: { rowIndex: 3, columnIndex: 1 },
			end: { rowIndex: 4, columnIndex: 4 },
		},
		styles: { background: "#f8f8f8", color: "#000000" },
	},
	{
		range: {
			start: { rowIndex: 1, columnIndex: 3 },
			end: { rowIndex: 3, columnIndex: 4 },
		},
		styles: { background: "#f8f8f8", color: "#000000" },
	},
];
...
export const ReactGridExample = () => {
	...
	const yearsDataWithTotals = yearsData.map((year) => ({
		...
		values: [
			...year.values,
			year.values.reduce((sum, value) => sum + value, 0),
		],
	}));
	...
	const summaryRow = {
		...
		values: yearsDataWithTotals.reduce(
			(sum, year) =>
				sum.map((currentSum, index) => currentSum + year.values[index]),
			[0, 0, 0]
		),
	};
